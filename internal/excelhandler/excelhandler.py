...
def save_data_to_excel(pd_data: DataFrame, writer: ExcelWriter, sheet_name:str, interesting_stock: set):
    ...
    pe_evaluation(pd_data.loc[:,['อัตราส่วนราคาต่อกำไร','กำไรต่อหุ้น','ราคา']].T, writer, workbook, worksheet, sheet_name,interesting_stock, marking=bool(os.getenv("MARK_PE")))
    ddm_evaluation(pd_data.loc[:, ['เงินปันผลต่อหุ้น', 'ราคา']].T, writer, workbook, worksheet, sheet_name,interesting_stock, marking=bool(os.getenv("MARK_DDM")))
